...
if dataset == "emotion":
    ...
    non_blocking = pin_mem

    train_dataset = EmotionDataset("train", block_size)
    train_dl = torch.utils.data.DataLoader(
        train_dataset, batch_size=batch_size, shuffle=True, pin_memory=pin_mem
    )
    val_dataset = EmotionDataset("validation", block_size)
    val_dl = torch.utils.data.DataLoader(
        val_dataset, batch_size=batch_size, shuffle=False, pin_memory=pin_mem
    )

    # stupid wrapper to make the rest of the code work
    def get_batch(split):
        if split == "train":
            batch = next(iter(train_dl))
            return batch["text"].to(device, non_blocking=non_blocking), batch[
                "label"
...
if init_from == "scratch":
    ...
    model = (
        ...
        else GPT4SentimentAnalysis.from_pretrained(init_from, override_args)
    )
